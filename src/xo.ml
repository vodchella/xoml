...
let process_input str =
    match str with
    | s when (is_valid_move_str s) -> Move str
    | "q" | "quit" | "exit" -> Quit
...
let rec input_loop (g: game) =
    ...
    print_at "?> " (input_vmargin + 1) 1;
    flush stdout;
    let input = read_line () in
    match process_input (String.trim input |> String.lowercase_ascii) with
    | Quit -> ()
    | Invalid inv ->
        let new_game = { last_tip = inv } in
        input_loop new_game
    | Move m ->
        let new_game = { last_tip = "Your move: " ^ m } in
        input_loop new_game
